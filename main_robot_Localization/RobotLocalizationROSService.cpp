...
{
  RobotLocalizationROSService robot;

  // Define Matrices for Frame A and Frame B and Transformation Matrix Tr
  Eigen::MatrixXd A(4, 3);
  Eigen::MatrixXd B(4, 3);
  Eigen::MatrixXd Tr(4, 4);

  // Read Frame A and B from text files
  A = robot.initializeMatrixFromInputFile("frame_a.txt");
  std::cout << "Here is the matrix B:\n" << A << std::endl;

  B = robot.initializeMatrixFromInputFile("frame_b.txt");
  std::cout << "Here is the matrix B:\n" << B << std::endl;

  // Calculate Homogeneous Transformation Matrix
  Tr = robot.calculateHomogeneousTransformationMatrix(A,B);
